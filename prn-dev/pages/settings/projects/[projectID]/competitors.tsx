...
@observer
export default class ProjectCompetitorsPage extends Component<IProps, any> {
    ...
    return (
      <ProjectSettingsLayout>
        ...
        <Row padding='xxl'>
          <Col size={12}>
            <TitleBlock
              title='Страницы конкурентов'
              text='Добавляйте любые аккаунты, группы и сообщества ваших конкурентов в социальных сетях. Можно добавить несколько страниц в каждую из социальных сетей.'
        ...
        {!profileStore.checkPermission(UserPermission.COMPETITORS, projectID) && profileStore.checkPermission(UserPermission.PROJECT, projectID) && (
          <>
            <Notification
              icon='admin'
        ...
        <ServiceBlockGroup size='m'>
          {ArrayUtil.arrayObjectsSort(router.query.sort, communitiesStore.getCompetitorsCommunities(projectID)).map(community => (
            <Community
              ...
              url={community.url}
              to={`https://prns.c-cube.ru/statistics?type=ONE&page=detail&accountID=${community.accountID}&reportCommunityID=${community.communityID}&token=${profileStore.token}`}
